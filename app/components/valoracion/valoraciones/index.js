...
const Valoraciones = () => {
  ...
  useEffect(() => {
    ...
    setUserId(storedUserId); // Guardamos el ID del usuario
        ...
        <ul className={styles.listaValoraciones}>
          {valoraciones.map((valoracion) => (
            <li key={valoracion.id} className={styles.valoracionItem}>
              <div className={styles.valoracionHeader}>
                <strong>{perfiles[valoracion.idusuario] || 'Usuario desconocido'}</strong>

                {/* Mostrar el botón solo si el ID de localStorage es igual a idusuario */}
